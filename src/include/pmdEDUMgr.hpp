...
private:
   ...
   std::map<unsigned int, EDUID> _tid_eduid_map; // we can find EDU with thread ID
...
public:
   ...
   EDUID getSystemEDU(EDU_TYPES edu) {
      ...
      _mutex.get_shared();
      auto it = _mapSystemEDUS.find(edu);
   ...
   static bool isPoolable(EDU_TYPES type) { // only agent is allowed to go back to thread pool. This should never be called.
...
private:
    ...
    * _PMD_EDU_WAITING or PMD_EDU_IDLE or PMD_EDU_CREATING status
    ...
    * idle queue, representing the EDU is pooled (no longer associate with
    * any user activities)
